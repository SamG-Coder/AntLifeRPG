...
## Ceiling-anchored tapered fine roots

Fine hanging roots now start on projected cave-ceiling contacts, embed slightly into the soil, curve in varied directions and taper to small tips. Their lengths retain walking headroom instead of using arbitrary endpoints near the floor. Selected roots branch into finer tapered offshoots. The three large roots and their camera envelopes retain their previous shapes. The fine-root loop consumes the same five random values per candidate, preserving downstream decoration placement.
...
## Routes to available parcels

Food duties now select the shortest reachable available seed through a small visibility graph over the player and chamber centres, with ground segments sampled against the actual walkable footprint. Nearby dropped seeds can be approached directly instead of forcing a trip through the root garden. Nursery cleanup uses the same search for a collectable soil load. Journal destinations remain place routes. Pickup or drop cancels the current trail, and an active parcel trail reselects if its target moves, becomes owned or otherwise ceases to be available.

All 84 tests, lint and build pass. Regression coverage includes local recovery after actual pickup/drop, densely sampled route segments between the real chambers and rear nursery parcels, and exclusion of owned, deposited, falling, player-carried and unreachable loads. Browser playtesting followed the last surface seed, carried it home, dropped it near (0,-0.24), then followed its duty scent locally to approximately (0,0.54), where the pickup prompt appeared. The player remained in the home chamber. The scene ran near 60 FPS with no captured warnings/errors.

This uses static ground visibility and chamber waypoints, not a full navigation mesh or body-aware path planner. It does not reserve a parcel, guarantee avoidance of dynamic crowds, replenish food or route attached/climbing ants. Target invalidation is covered through selection tests and code integration; a live worker stealing the player's active target was not observed in this browser session.

## Persistent morning food supply

A simulation-owned caretaker event replenishes the root garden at 06:00 once per colony day, up to 12 undeposited seeds including carried/owned loads. A persisted day marker prevents duplicate arrivals after reload. Existing saves adopt the schedule without an immediate extra scatter. A low shared reserve (six portions or fewer) releases the three-seed teaching reserve so foragers can gather every remaining available seed.

The supply record tracks arrivals and archived display parcels. At each morning event, already-credited deposited seed displays are reduced to the latest 24; cumulative delivery counters and shared food remain intact. Loose, owned and carried loads are preserved. The renderer removes meshes whose archived item records no longer exist, using shared geometry/materials without disposing them. Field notes describe the schedule, and active/rest summaries report arrivals.
...
## Distinct worker gathering targets

Workers now retain a parcel target while walking to gather it. Other workers choose a different seed, nursery leaf, or soil load instead of converging on that same item. This saved intention remains separate from cargo ownership: the item stays on the ground until pickup, and the player can still take it. Workers discard invalid targets, update approaches when a loose item moves, and release targets on rest or meal breaks. Save validation rejects duplicate worker targets. The nursery's legacy approach migration no longer redirects an active collector's parcel route to a waiting place.

All 107 tests, lint and build pass. New regressions verify six different seed targets while preserving three unclaimed seeds, six distinct nursery-leaf targets, target persistence, player pickup taking precedence, moved-item approaches, break-time release, and duplicate-target rejection. Existing excavation-to-lining and 20-day food regressions remain green. Shared job-availability queries are cached for workers with equivalent unassigned roles, while assigned targets get separate queries.
